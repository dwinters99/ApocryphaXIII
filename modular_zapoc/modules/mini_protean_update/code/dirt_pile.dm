...
/obj/effect/decal/dirt_pile/Initialize(mapload)
	. = ..()
	var/turf/turf_to_copy = get_turf(src)
	add_atom_colour(turf_to_copy.turf_mask_color, FIXED_COLOUR_PRIORITY)


/obj/effect/decal/dirt_pile/proc/remove_dirt_pile()
	animate(src, alpha = 0, time = 2 SECONDS) // Fade out
	spawn(3 SECONDS)
		qdel(src)
